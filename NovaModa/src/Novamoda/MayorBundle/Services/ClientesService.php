...
class ClientesService
{
     ...
     */
    public function obtenerClientesPaginado($paginacion, $array)
    {

        $result = new \Novamoda\MayorBundle\Model\ResultPaginacion();
        $repo = $this->em->getRepository('NovamodaMayorBundle:Clientes');
        $query = $repo->createQueryBuilder('alm');
        $query = $repo->filtrarDatos($query, $array);
        if (!is_null($paginacion->contiene)) {
            $query = $repo->consultaContiene($query, ["nombre", "codigo", "apellido"], $paginacion->contiene);
        }
