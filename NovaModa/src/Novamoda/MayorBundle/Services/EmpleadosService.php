...
class EmpleadosService
{
     ...
     */
    public function obtenerEmpleadosPaginado($paginacion, $array)
    {

        $result = new \Novamoda\MayorBundle\Model\ResultPaginacion();
        $repo = $this->em->getRepository('NovamodaMayorBundle:Empleados');
        $query = $repo->createQueryBuilder('alm');
        $query = $repo->filtrarDatos($query, $array);
        if (!is_null($paginacion->contiene)) {
            $query = $repo->consultaContiene($query, ["nombres", "codigo", "apellidos"], $paginacion->contiene);
        }
