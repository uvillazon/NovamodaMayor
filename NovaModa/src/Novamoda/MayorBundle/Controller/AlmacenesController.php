...
class AlmacenesController extends BaseController
{
     ...
     * )
     */
    public function getAlmacenesAction(Request $request)
    {
        $paginacion = $this->obtenerPaginacion($request);
        /**
         * @var \Novamoda\MayorBundle\Services\ProformasService $servicio
         */
        $servicio = $this->get('mayorbundle.almacen_service');
        $array = $request->query;
        $result = $servicio->obtenerAlmacenesPaginado($paginacion, $array);
        return $result;
    }
